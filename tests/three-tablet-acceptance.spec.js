...
for(const height of [734,689])test(`Tablet Optimized real GPU acceptance: 1180x${height}`,async({page},info)=>{
 ...
 await page.addInitScript(()=>{
  ...
  wrap(GPUDevice.prototype,'createShaderModule',(fn,self,args)=>{
   const module=Reflect.apply(fn,self,args);if(args[0].label==='fragment_GTAO')e.gtao++;
  ...
  wrap(GPUDevice.prototype,'createTexture',(fn,self,args)=>{const d=args[0],result=Reflect.apply(fn,self,args);if(d.usage&GPUTextureUsage.RENDER_ATTACHMENT)e.textures.push({operation,phase,label:d.label,format:d.format,size:d.size,actualSize:[result.width,result.height],samples:result.sampleCount});return result;});
 ...
 try{
  for(let entry=0;entry<2;entry++){
   const before=await page.evaluate(()=>({operations:window.tabletQA.operations.length,fences:window.tabletQA.fences}));
   await choose('3d');
   ...
   expect(s.status).toBe('ready');expect(s.backend).toBe('WebGPU');expect(s.resolution).toEqual([1180,height]);
   expect(s.configuration.preset).toEqual({id:'tablet-optimized',name:'Tablet Optimized',worldSamples:1,effectSamples:1,dprCap:1,shadowSize:2048,volumeResolution:.25,volumeSteps:40,gtao:false,bloom:true,anisotropy:4});
   expect(s.configuration.presentationSamples).toBe(1);expect(s.configuration.presentationDepth).toBe(false);
   expect(s.actualEffects).toEqual({gtao:false,bloom:true,volumeSteps:40,volumeResolution:.25});
   ...
   expect(e.errors).toEqual([]);expect(e.gtao).toBe(0);expect(Math.max(...e.anisotropy)).toBe(4);
   expect(e.textures.some(t=>t.format.startsWith('depth')&&t.size.width===2048&&t.size.height===2048)).toBe(true);
   const effects=e.pipelines.filter(p=>/^Warm-up/.test(p.phase));
   expect(effects.length).toBeGreaterThan(5);expect(effects.every(p=>p.samples===1)).toBe(true);
