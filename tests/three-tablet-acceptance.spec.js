...
for(const height of [734,689])test(`Tablet Optimized real GPU acceptance: 1180x${height}`,async({page},info)=>{
 ...
 try{
  for(let entry=0;entry<2;entry++){
   ...
   expect(s.actualEffects).toEqual({gtao:false,bloom:false,volumeSteps:0,volumeResolution:0,fxaa:true,lightingMode:"golden-evening",sunDirection:[-38,28,24],forestFloor:true});
