...
for(const action of ['failure','cancel'])test(`warm-up ${action} releases the GPU device and allows Cinematic Lighting`,async({page},info)=>{
 ...
 await page.addInitScript(action=>{
  ...
  window.destroyedDevices=0;window.triggered=false;
 ...
 await expect.poll(()=>page.evaluate(()=>window.eval('threeRenderer.snapshot')().status)).toBe(action==='failure'?'error':'idle');
 expect(await page.evaluate(()=>window.destroyedDevices)).toBe(1);
 if(action==='failure'){
  await expect(page.locator('[data-three-diagnostic]')).toContainText('RangeError: Range consisting');
