(function () {
  ...
  const events=new WeakMap();let trace=null,lastError='',currentPreparation='',inputStatus='',visibleStatus='',gpuStatus='';
  ...
  const render=()=>{panel.textContent=[gpuStatus,inputStatus,visibleStatus,previousPreparation,currentPreparation,(!inputStatus&&trace?`Tapdiagnose · ${trace.button}\n${['pointerdown','touchstart','click'].map(type=>`${type}: ${trace.seen[type]||'—'}`).join(' · ')}\n${trace.hit}\n${trace.style}\n${trace.lines.join('\n')}`:''),lastError?'FOUT: '+lastError:''].filter(Boolean).join('\n');};
