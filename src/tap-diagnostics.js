(function () {
  ...
  const events=new WeakMap();let trace=null,lastError='',currentPreparation='',presetStatus='',inputStatus='',visibleStatus='',gpuStatus='';
  ...
  const render=()=>{panel.textContent=[presetStatus,gpuStatus,inputStatus,visibleStatus,previousPreparation,currentPreparation,(!inputStatus&&trace?`Tapdiagnose · ${trace.button}\n${['pointerdown','touchstart','click'].map(type=>`${type}: ${trace.seen[type]||'—'}`).join(' · ')}\n${trace.hit}\n${trace.style}\n${trace.lines.join('\n')}`:''),lastError?'FOUT: '+lastError:''].filter(Boolean).join('\n');};
  ...
  window.addEventListener('atlas-three-preparation',event=>{const record=event.detail;presetStatus=record.rendererSettings||'';currentPreparation=`3D (${record.strategy}, ${record.state}): ${record.operation}\nLaatst voltooid: ${record.lastCompleted||'—'}`;render();});
