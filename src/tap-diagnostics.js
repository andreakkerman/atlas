(function () {
  ...
  const render=()=>{panel.textContent=[presetStatus,rangeStatus,gpuStatus,inputStatus,visibleStatus,previousPreparation,currentPreparation,(!inputStatus&&trace?`Tapdiagnose · ${trace.button}\n${['pointerdown','touchstart','click'].map(type=>`${type}: ${trace.seen[type]||'—'}`).join(' · ')}\n${trace.hit}\n${trace.style}\n${trace.lines.join('\n')}`:''),lastError?'FOUT: '+lastError:''].filter(Boolean).join('\n');};
