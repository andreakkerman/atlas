(function () {
  ...
  let panel=null;
  window.AtlasTapDiagnostics={setEnabled(enabled){
    if(enabled&&!panel)initialize();
  ...
  window.addEventListener('atlas-display-status',event=>{
    const s=event.detail;
    runtimeStatus=`${s.mode} · ${s.status}\n${Number.isFinite(s.fps)?Math.round(s.fps):'—'} FPS · CPU ${Number.isFinite(s.cpu)?s.cpu.toFixed(1):'—'} ms\n${s.preparation||''}`;
    render();
  });
  let runtimeStatus='';
  const events=new WeakMap();let trace=null,lastError='',currentPreparation='',presetStatus='',rangeStatus='',inputStatus='',visibleStatus='',gpuStatus='';
  ...
  const render=()=>{if(panel.hidden)return;panel.textContent=[runtimeStatus,presetStatus,rangeStatus,gpuStatus,inputStatus,visibleStatus,previousPreparation,currentPreparation,(!inputStatus&&trace?`Tapdiagnose · ${trace.button}\n${['pointerdown','touchstart','click'].map(type=>`${type}: ${trace.seen[type]||'—'}`).join(' · ')}\n${trace.hit}\n${trace.style}\n${trace.lines.join('\n')}`:''),lastError?'FOUT: '+lastError:''].filter(Boolean).join('\n');};
  window.addEventListener('atlas-three-gpu',event=>{const g=event.detail;if(g.operation==='3D gereed'&&!g.firstFailure&&!g.loss){gpuStatus='';render();return;}const s=g.firstFailure?.shader||g.lastShader,e=g.firstFailure?.error;const f=g.firstFailure;gpuStatus=`${f?`Foutbron: ${f.type} · pass: ${f.pass||"—"}\n${f.copy?JSON.stringify(f.copy)+"\n":""}${f.error.stack?.split("\n").filter(line=>!line.includes("three-gpu-diagnostics.js")).slice(0,4).join("\n")||""}\n`:""}GPU ${g.operation}\nVerlies: ${g.loss?`${g.loss.reason} · ${g.loss.message} · na opruimen=${g.loss.cleanupAlreadyRequested}`:'nog niet gemeld (geen bewijs van geldigheid)'}\nLaatste shader (niet per se foutbron): ${s?.label||'—'} · ${s?.hash||'—'} · nieuw=${s?.newSource??'—'}\nPass: ${s?.pass||'—'} · object: ${s?.object?.name||'—'} · materiaal: ${s?.object?.material||'—'}\nModules=${g.counts.shaders} pipelines=${g.counts.pipelines}${e?`\n${e.name}: ${e.message}`:''}`;render();});
  window.addEventListener('atlas-three-input',event=>{const s=event.detail;inputStatus=`Input: ${s.transport} · move=${s.moveId??'—'} look=${s.lookId??'—'}\nvector=${s.walk.toFixed(2)},${s.turn.toFixed(2)} · toegestaan=${s.allowed} · x=${s.player.x.toFixed(1)}\n${s.lastEvent}; capture=${s.capture}\nstart/move/end/cancel=${s.counts.down}/${s.counts.motion}/${s.counts.end}/${s.counts.cancel} · reset=${s.lastReset}\ntoegepast=${s.movement.applied} (${s.movement.from?.toFixed(1)??'—'} → ${s.movement.to?.toFixed(1)??'—'}) · guards=${JSON.stringify(s.guards)}`;render();});
  window.addEventListener('atlas-three-visible-profile',event=>{const p=event.detail,s=p.samples.at(-1);if(!s)return;const c=s.counts;visibleStatus=`Eerste beelden ${s.seconds}s: ${s.fps??'—'} FPS, CPU ${s.cpuMs??'—'}ms, max frame ${s.maxFrameMs}ms\nGPU-wacht=${s.gpuQueueWaitMs??'—'}ms · pipelines=${(c.createRenderPipeline||0)+(c.createRenderPipelineAsync||0)} textures=${c.textures||0} mipmaps=${c.mipmaps||0}\nNPC=${c.npcUpdates||0} uploads=${c.copyExternalImageToTexture||0} schaduw=${c.shadowRefresh||0} · GC niet meetbaar\n${p.unavailable.length?'Niet meetbaar: '+p.unavailable.join(','):''}`;render();});
  window.addEventListener('atlas-three-preparation',event=>{const record=event.detail,f=record.failure;presetStatus=record.rendererSettings||'';rangeStatus=f?`Foutbron: ${f.pass||f.operation||'voorbereiding'} · ${f.copy?.api||f.name}\n${f.copy?JSON.stringify(f.copy)+'\n':''}${f.stack?.split('\n').filter(line=>!line.includes('three-gpu-diagnostics.js')).slice(0,5).join('\n')||f.message}`:'';currentPreparation=`3D (${record.strategy}, ${record.state}): ${record.operation}\nLaatst voltooid: ${record.lastCompleted||'—'}`;render();});
  const add=(entry,line)=>{entry.lines.push(line);entry.lines=entry.lines.slice(-8);if(entry===trace)render();};
  ...
  if(new URLSearchParams(location.search).get('debug3d')==='1')window.AtlasTapDiagnostics.setEnabled(true);
})();
