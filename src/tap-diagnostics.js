(function () {
  ...
  window.addEventListener('atlas-three-gpu',event=>{const g=event.detail;if(g.operation==='3D gereed'&&!g.firstFailure&&!g.loss){gpuStatus='';render();return;}const s=g.firstFailure?.shader||g.lastShader,e=g.firstFailure?.error;gpuStatus=`GPU ${g.operation}\nVerlies: ${g.loss?`${g.loss.reason} · ${g.loss.message} · na opruimen=${g.loss.cleanupAlreadyRequested}`:'nog niet gemeld (geen bewijs van geldigheid)'}\nShader: ${s?.label||'—'} · ${s?.hash||'—'} · nieuw=${s?.newSource??'—'}\nPass: ${s?.pass||'—'} · object: ${s?.object?.name||'—'} · materiaal: ${s?.object?.material||'—'}\nModules=${g.counts.shaders} pipelines=${g.counts.pipelines}${e?`\n${e.name}: ${e.message}`:''}`;render();});
