...
Ambient animals carry unique instance IDs, type/optional label, world x/y, positive scale, open/closed frame paths, sound and blink/cooldown settings. Appearance/mirroring fields use the existing normalizers. Flybys carry unique instance IDs, required frameA and optional/null frameB (static when absent), optional sound, path points, scale, speed, flap frequency, facing/mirroring, interval bounds and optional `syncKey`/`startDelayMs`; supported motion-profile controls extend that same instance. Optional `soundTriggers` is an array containing any combination of `"during"` and `"tap"`, including neither. It supersedes the compatible legacy `soundTrigger` string; missing configuration defaults to During only. Their off-world path points are intentional, unlike ordinary walking-path points. The dev server validates these payloads; do not assume the level CLI duplicates every endpoint check. Effect instance/group schema and geometry are owned by [Editor and Effects](EDITOR_AND_EFFECTS.md#procedural-scene-effects).
