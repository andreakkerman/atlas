...
**Sound Trigger** saves `soundTrigger: "during" | "tap"` through the normal draft/Apply path. Omitted means **During Flyby**: legacy trigger-time playback, same-path deduplication within a trigger, and the existing progress-based fade. **On Tap** flies silently and plays only when its active visible bounds are clicked/tapped in gameplay. Its audio uses the same flyby audio tracking, master × instance volume and lifecycle cleanup, without the flight-progress fade; repeated taps on an already playing path are ignored. Inactive/offscreen instances have no usable hit target. Editor preview remains silent for On Tap and does not introduce another interaction mode; close the editor to test taps. No new audio channel or per-level exception is used.
