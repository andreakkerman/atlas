...
The playable sprite set is selected per level through the shared character registry (see the Level Contract). Decode the selected set before revealing the player; retain the shared actor slot and controller. Cinematic/Voxel texture caches use the actual frame URL, and their interim frame fallbacks are scoped to the selected character ID so a sprite-set switch cannot reuse the previous character's image.

Cinematic uploads sprites at their decoded source dimensions. Its two main HDR targets and presentation canvas use the viewport's device-pixel resolution, uniformly reduced only if the device's `maxTextureDimension2D` requires it. Do not independently cap width/height or lower the DPR: characters share the scene targets, so subsequent browser upscaling loses detail even with relighting/wrap/rim disabled. Bloom retains separate quarter-resolution targets; character relighting and edge treatments remain in the full-resolution pass. This increases Retina fill rate and target memory compared with the former DPR-1.5/1920×1440 cap; verify both orientations on physical iPad when assessing performance. Three/Voxel resolution policies are separate and unchanged.
...
The shared Cinematic/Voxel broker separately tracks `uninitialized`, `initializing`, `ready`, `stale`, `lost`, `recovering` and `failed`, plus whether a device previously worked. Illustrated uses this broker whenever a selected authored capability contributes; without active selected capabilities it does not acquire WebGPU. Missing API or unsupported required features/limits fails immediately. Adapter-null and device-acquisition failures get four attempts: immediate, then waits of 250, 750 and 1500 ms. Each attempt tries high-performance then default adapter selection, and reacquires the adapter after device failure. Native acquisition waits are bounded to four seconds each. Only the exhausted failure is exposed to the existing playable DOM fallback/error banner; a successful recovery emits no unavailable warning. Exhaustion is latched to avoid retry storms; a new level preparation or foreground lifecycle transition permits another transient attempt, while missing API/unsupported requirements remain failed for the session.

There is one shared acquisition promise. Hiding/pagehide, menu cancellation and level replacement invalidate it; uncancellable late native devices are destroyed instead of registered. Hidden pages never initiate shared renderer work. A retained device is marked for revalidation on suspension; the next applicable renderer synchronization checks its queue completion (bounded to four seconds). Healthy resources are retained. Loss or failed revalidation invalidates the broker device/adapter and each renderer uses its own disposal path before reinitialization. Cinematic releases pipelines, buffers, bind groups, samplers, depth cache, scene targets and uploaded textures, including dynamic Flyby textures. Flyby decoded frames and source canvases stay with their existing asset/runtime owners and upload again to the new device. For GPU-requiring features, the existing asset plan also preloads the optional scene depthmap; Cinematic reuses prepared background/depth images without creating another decoded-image cache. Missing depth keeps the existing flat fallback. Generation checks reject stale decode/pipeline/presentation callbacks. Three retains its separate device and quality/preparation policy. If hidden while still preparing, its existing suspend path invalidates that preparation before a cancelled shared-device handoff can start new GPU work; foreground visibility resumes preparation. Already-ready Three worlds retain their ordinary visibility pause/resume behavior.
...
Illustrated and Cinematic use the same `cinematicLighting.particles` level settings, normalization, packing, presets, layer switches and GPU particle vertex/fragment shader in `cinematic-renderer.js` / `cinematic-shaders.js`. There are no per-mode particle parameter defaults or overrides; the Illustrated participation gate defaults on. The existing editor and world-config Apply path own all controls. Legacy Canvas2D scene effects remain separate and unchanged.

The single runtime has two presentation paths. Cinematic retains its HDR additive particle pass and existing finishing. With particles alone, Illustrated lazily compiles only the particle pipeline and draws directly to a viewport-sized GPU canvas, screen-composited over the DOM artwork. Black is neutral in this additive presentation; particles do not replace artwork, actors or traditional effects. The canvas lives in the world track above foreground effects and below interaction markers; its viewport placement cancels the track's camera translation, while the shared shader projects world coordinates. HUD, dialogue, companions and editor controls remain above it. The particle-only path does not allocate scene targets, sprite/artwork uploads, shadows, bloom or exposure resources; selected lighting and shadows use the minimal paths described below.
...
## Selective Illustrated capabilities

Graphics exposes `levels[id].illustratedFeatures`; [Editor and Effects](EDITOR_AND_EFFECTS.md#selective-illustrated-graphics) owns its defaults, UI and persistence. The shared settings contract creates a runtime-only projection of existing authored Cinematic data. Never save that projection over `cinematicLighting`.

All disabled means no Illustrated GPU scheduler or resources. Particle-only retains the direct shared particle pipeline, screen composite and zero scene targets. Character Shadows without global world contributions uses the existing sprite/shadow shader, one cropped linear artwork receiver target and a transparent canvas below the DOM actors. Optional particles use the same owner/device/depth texture and an independent additive presentation canvas; there is still one scheduler. It does not compile a field, grading, bloom or exposure pass. Grounding supports the playable character (both sprite sets) and existing NPC sprites with their authored opt-outs; animals and Flybys receive no shadows. The shadow system samples receiver artwork and sprite alpha, not scene depth.

When selected authored grading, area lighting or depth perspective contributes, the shared world passes composite artwork and actors at display DPR. Only the required field pass is compiled; grading and depth perspective use the existing finish shader. Two linear scene targets preserve the pre-shadow receiver and finished world; bloom targets and exposure adaptation are absent. Relighting, wrap, rim, shafts, God Rays, local-light Effects, atmosphere and water Effects are excluded from this projection. Traditional Illustrated effect canvases remain authored and scheduled by their original runtime and participate in the world composite. HUD, companion/challenge panels, debug text and editor overlays remain DOM layers outside grading.

Scene Depth gates the authored perspective contribution. Resource availability is separate: enabled particle masking and area-light spatial guidance can still require the single depth texture with perspective off. Flyby masking retains its existing ambient depth owner. The resource recipe responds to live changes, releases unused resources and preserves authored choices across mode changes. The shared broker owns acquisition/revalidation/recovery; no second adapter/device is introduced. Device loss rebuilds selected pipelines, scene targets, shadow and particle resources and depth bindings through the existing preparation cover.

Illustrated retains its traditional effects and excludes Cinematic finishing. Its additive particle presentation and transparent DOM shadow composition can differ from Cinematic's linear HDR composition; field definitions, grading parameters, depth conventions and grounding controls are shared. Physical iPad frame pacing, memory, Safari visual quality, and installed-PWA suspend/resume remain device verification requirements.
